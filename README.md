...
- Pre-computed lookup tables (generated at build time and embedded in the binary; see [Generation](#generation) for how they're built)
...
Tables (one per group, plus Kociemba's two phases) aren't checked in; they're generated by [`build.rs`](./build.rs) - which depends on the generation code in the `thwaite-core` workspace member - into `OUT_DIR` the first time the crate is built, then embedded with `include_bytes!`. A clean build takes a few minutes because of this; subsequent builds skip regeneration as long as `OUT_DIR` still holds the files (i.e. `cargo clean` forces it to happen again).
