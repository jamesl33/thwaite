...
fn path(out_dir: &str, name: &str) -> Option<PathBuf> {
    ...
    println!("cargo:warning=thwaite: generating {name}, this may take a while...");
...
fn main() {
    ...
    if let Some(path) = path(&out_dir, "group_two_table.db") {
        ...
        // representatives (see `group_two::initial`), not from the solved cube alone, so the solved cube's own
        // coordinate doesn't necessarily land on that coordinate's depth-0 anchor the way it does for the other
        // five (single-seed) tables.
